...
for i=1:4
    load_name = sprintf('acc_fpn_wine_rule15_nl_%s_epoch_200_all.mat', noise_list{1,i}); 
    load(load_name);
...
save gsad_mean_std.mat test_acc_mean test_acc_std;
for j=1:size(algorithm_list, 2)
    fprintf('%s Acc: & %.2f/%.2f & %.2f/%.2f & %.2f/%.2f & %.2f/%.2f \n',...
            algorithm_list{1,j}, test_acc_mean(j,1), test_acc_std(j,1), test_acc_mean(j,2), test_acc_std(j,2),...
