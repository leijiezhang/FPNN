...
from models.model_run import gp_cmp
import torch
import os
...
def run_gp(param_config: ParamConfig):
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        # dgp saving tensors
        gp_train_acc_tsr = torch.empty(10, 0)
        gp_test_acc_tsr = torch.zeros(10, 0)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            gp_train_acc, gp_valid_acc = gp_cmp(param_config, train_data, test_data, kfold_idx)

            gp_test_acc_tsr = torch.cat([gp_test_acc_tsr, gp_valid_acc], 1)
            gp_train_acc_tsr = torch.cat([gp_train_acc_tsr, gp_train_acc], 1)
        ...
        save_dict = dict()
        # save gp results
        save_dict["gp_test_acc_tsr"] = gp_test_acc_tsr.cpu().numpy()
        save_dict["gp_train_acc_tsr"] = gp_train_acc_tsr.cpu().numpy()

        data_save_file = f"{data_save_dir}/acc_gp_{param_config.dataset_folder}" \
