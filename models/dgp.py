...
from models.model_run import dgp_cmp_mthds
import torch
import os
...
def run_dgp(param_config: ParamConfig):
    ...
    param_config.log.info(f"dgp epoch_size : {param_config.n_epoch_dgp}")
    param_config.log.info(f"noise level : {param_config.noise_level}")
    param_config.log.info(f"dgp lr : {param_config.lr_dgp}")
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        dgp21_train_acc_tsr = torch.empty(param_config.n_epoch_dgp, 0).to(param_config.device)
        dgp21_test_acc_tsr = torch.zeros(param_config.n_epoch_dgp, 0).to(param_config.device)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            dgp21_train_acc, dgp21_valid_acc = dgp_cmp_mthds(param_config, train_data, test_data, kfold_idx)
