...
from models.model_run import run_fpn_model
import torch
import os
import scipy.io as io


def run_fpn(param_config: ParamConfig):
    """
    todo: run the codes
    ...
    param_config.log.info(f"this model is a comparison of fpn and dnn model")
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            fpn_train_acc, fpn_valid_acc = run_fpn_model(param_config, train_data, test_data, kfold_idx)
        ...
        data_save_file = f"{data_save_dir}/acc_fpnl_{param_config.dataset_folder}" \
